...
async function carrierDashboard(){
 ...
 $("#carrierForm").onsubmit=async e=>{
   ...
   try{
     const face=await uploadCarrierPhoto(d.face_photo,"face",u.id);
     const idCard=await uploadCarrierPhoto(d.id_card_photo,"id-card",u.id);
     const driverDoc=await uploadCarrierPhoto(d.driver_document_photo,"driver-document",u.id);
     const vehicle=await uploadCarrierPhoto(d.vehicle_photo,"vehicle",u.id);
     const r=await sb.from("carrier_profiles").upsert({id:u.id,vehicle_type:d.vehicle_type,vehicle_make:d.vehicle_make,vehicle_year:Number(d.vehicle_year),vehicle_condition:d.vehicle_condition,capacity:d.capacity,base_zone:d.base_zone,cargo_types:(d.cargo_types||"").split(",").map(x=>x.trim()).filter(Boolean),service_types:(d.service_types||"").split(",").map(x=>x.trim()).filter(Boolean),vehicle_description:d.vehicle_description||null,face_photo_path:face,id_card_photo_path:idCard,driver_document_photo_path:driverDoc,vehicle_photo_path:vehicle,verification_status:"pending",return_alerts:d.return_alerts==="true"});
     if(r.error)throw r.error;
     const pr=await sb.from("profiles").update({identity_attested:true,identity_attested_at:new Date().toISOString()}).eq("id",u.id);

...
function loadForm(){
 ...
 $("#loadForm").onsubmit=async e=>{e.preventDefault();const u=await user(),d=Object.fromEntries(new FormData(e.target));const r=await sb.from("load_requests").insert({customer_id:u.id,cargo_type:d.cargo_type,origin_area:d.origin,destination_area:d.destination,requested_date:d.pickup_date,quantity:d.quantity,urgency:(d.urgency||"normal").toLowerCase(),budget:d.budget?Number(d.budget):null,notes:d.notes||null,status:"open"});if(r.error){alert(r.error.message);return}openModal('<div class="success"><div class="success-icon">✓</div><span class="eyebrow">PUBLICADA</span><h2>Tu carga ya está en MOVELO.</h2><p>Ahora un transportista compatible puede verla y cotizar.</p><button class="primary submit" id="ok">Ver mis cargas</button></div>');$("#ok").onclick=customerDashboard;refresh()}
}
...
async function carrierOperations(){
 ...
 for(const q of r.data||[]){const l=q.load_requests;
   h+='<article class="dash-card"><div><b>'+esc(l?.cargo_type||"Carga")+'</b><span class="status">'+esc(q.status)+'</span></div><strong>'+esc(l?.origin_area||"")+' → '+esc(l?.destination_area||"")+'</strong><small>'+esc(l?.quantity||"")+' · '+esc(l?.requested_date||"")+' · '+esc(l?.urgency||"")+'</small><p>Tu cotización: <b>

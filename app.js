...
function loadForm(){
 ...
 $("#loadForm").onsubmit=async e=>{
   ...
   if(d.identity_attested==="true"){
     const pr=await sb.from("profiles").update({identity_attested:true,identity_attested_at:new Date().toISOString()}).eq("id",u.id);
...
async function carrierDashboard(){
 ...
 openModal('<span class="eyebrow">TRANSPORTISTA · VERIFICACIÓN</span><h2>Primero verificamos quién eres.</h2><p class="modal-sub">Para ofrecer transporte en MOVELO necesitamos identificarte y conocer el vehículo o maquinaria que utilizas. Los documentos de verificación son confidenciales y no se mostrarán públicamente ni a otros usuarios.</p><div class="privacy-box">🔒 <strong>Datos confidenciales:</strong> tu licencia fotografiada junto a tu cara se usa para verificar identidad y autorización para conducir. Las fotos del vehículo o maquinaria se usan para verificar el equipo y mostrarlo en el perfil. La documentación de identidad no se publica.</div><form id="carrierForm"><label>Licencia + tu cara, en una sola foto <input name="license_face_photo" type="file" accept="image/*" capture="environment" required></label><small class="form-note">Tómate una foto donde se vea claramente tu cara y tu licencia.</small><label>Fotos del vehículo / maquinaria <input name="vehicle_photo" type="file" accept="image/*" capture="environment" multiple required></label><small class="form-note">Sube exactamente 2 fotos: de frente, de lado o desde dos ángulos. Las 2 son obligatorias.</small><label>¿Qué tienes para trabajar?<select name="vehicle_type" required><option value="">Elige</option><option>Carro</option><option>Pickup</option><option>Panel / furgón</option><option>Camión</option><option>Camión refrigerado</option><option>Mula / tractocamión</option><option>Plataforma</option><option>Grúa</option><option>Moto</option><option>Bus / transporte de personas</option><option>Retroexcavadora</option><option>Bulldozer / cuchilla</option><option>Tractor agrícola</option><option>Cosechadora / maquinaria agrícola</option><option>Montacargas</option><option>Otra maquinaria terrestre</option></select></label><div class="two"><label>Marca<input name="vehicle_make" required placeholder="Ej. Toyota"></label><label>Año<input name="vehicle_year" type="number" min="1950" max="2026" required placeholder="Ej. 2018"></label></div><label>Estado del vehículo / maquinaria<select name="vehicle_condition" required><option value="">Elige</option><option>Excelente</option><option>Bueno</option><option>Regular</option><option>Requiere mantenimiento</option></select></label><div class="two"><label>Capacidad<input name="capacity" placeholder="Ej. 2,000 kg"></label><label>Zona base<input name="base_zone" placeholder="Ej. Penonomé"></label></div><label>Servicios que puedes ofrecer<input name="service_types" placeholder="Ej. carga, mudanza, acarreo, trabajo agrícola"></label><label>Describe tu vehículo o maquinaria<input name="vehicle_description" placeholder="Modelo, capacidad, implementos u otros detalles"></label><label>Tipos de carga que manejas<input name="cargo_types" placeholder="Ej. agrícola, seca, animales, mudanza"></label><label>¿Quieres recibir oportunidades de retorno?<select name="return_alerts"><option value="true">Sí</option><option value="false">No por ahora</option></select></label><label class="consent-check"><input name="identity_attested" type="checkbox" value="true" required> <span><strong>Declaración de veracidad y autorización:</strong> Declaro que toda la información y documentación que proporciono es correcta, auténtica y veraz, corresponde a mi persona y que soy quien digo ser. Me comprometo a mantenerla actualizada y entiendo que MOVELO podrá revisarla exclusivamente para verificar mi identidad, seguridad y perfil de transportista. Estos datos son confidenciales y no se mostrarán públicamente ni a otros usuarios, salvo obligación legal o autorización expresa.</span></label><button class="primary submit">Guardar perfil y solicitar verificación →</button><small class="form-note">🔒 Tu perfil quedará pendiente de verificación antes de operar como transportista.</small></form>');
 $("#carrierForm").onsubmit=async e=>{
   e.preventDefault();
   const d=Object.fromEntries(new FormData(e.target));
   if(d.identity_attested!=="true"){alert("Debes aceptar la declaración de veracidad para continuar.");return}
   if(validatePublicText({capacity:d.capacity,base_zone:d.base_zone,service_types:d.service_types,vehicle_description:d.vehicle_description,cargo_types:d.cargo_types}))return rejectConfidentialData();
   ...
   try{
     ...
     carrierLoads();
   }catch(err){alert(err.message||"No se pudieron guardar los documentos y el perfil.")}
 };
}
async function carrierOperations(){
 ...
 for(const q of r.data||[]){const l=q.load_requests;
   ...
   if(q.status==="accepted"&&l?.status==="accepted")h+='<button class="secondary small chatBtn" data-load="'+q.load_id+'">Abrir chat</button><button class="secondary small completeBtn" data-load="'+q.load_id+'">Marcar mi parte completada</button>';
 ...
 document.querySelectorAll(".completeBtn").forEach(b=>b.onclick=completeTrip);
}
async function opportunityDetails(loadId){ const r=await sb.from("load_requests").select("*").eq("id",loadId).maybeSingle(); if(r.error||!r.data){alert(r.error?.message||"Oportunidad no encontrada");return} const l=r.data;if(validatePublicText({origin:l.origin_area,destination:l.destination_area,quantity:l.quantity,notes:l.notes,cargo_detail:l.cargo_detail,special_requirements:l.special_requirements}))return rejectConfidentialData(); const vehicleDetails=l.cargo_detail||l.special_requirements||"";openModal('<span class="eyebrow">OPORTUNIDAD MOVELO</span><h2>'+esc(l.origin_area)+' → '+esc(l.destination_area)+'</h2><p>'+esc(l.cargo_type)+' · '+esc(l.quantity)+' · '+esc(l.requested_date)+' · '+esc(l.urgency)+'</p>'+(vehicleDetails?'<p><strong>Servicio:</strong> '+esc(vehicleDetails)+'</p>':"")+'<p>'+esc(l.notes||"Sin detalles adicionales.")+'</p><p>🔒 Contacto protegido.</p><button class="primary submit" id="quoteFromOpp">Cotizar esta oportunidad →</button>'); $("#quoteFromOpp").onclick=()=>quoteForm(loadId); }
...
async function quoteForm(loadId){
 openModal('<span class="eyebrow">COTIZACIÓN</span><h2>¿Cuánto cobras por realizar este trabajo?</h2><p class="modal-sub" id="quoteVehicleInfo">Consultando tu vehículo…</p><form id="quoteForm"><label>Precio<input name="amount" type="number" step="0.01" required placeholder="Ej. 200"></label><label>Mensaje (opcional)<textarea name="note" rows="3" placeholder="Horario, condiciones, etc."></textarea></label><button class="primary submit">Enviar cotización →</button><small class="form-note">🔒 Tu teléfono no se comparte.</small></form>');
 const u=await user();const cp=await sb.from("carrier_profiles").select("vehicle_type,vehicle_make,vehicle_year,vehicle_condition").eq("id",u.id).maybeSingle();const vehicle=cp.data;
 $("#quoteVehicleInfo").innerHTML=vehicle?'<strong>Tu vehículo:</strong> '+esc(vehicle.vehicle_type||"Transporte")+' · '+esc(vehicle.vehicle_make||"Marca no indicada")+' · '+esc(vehicle.vehicle_year||"")+' · <strong>Estado: <span class="condition-'+String(vehicle.vehicle_condition||"").toLowerCase().replace(/[^a-záéíóúñ]+/g,"-")+'">'+esc(vehicle.vehicle_condition||"No indicado")+'</span></strong>':"Completa primero tu perfil de transportista.";
 $("#quoteForm").onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target));if(validatePublicText({note:d.note}))return rejectConfidentialData();const r=await sb.from("quotes").insert({load_id:loadId,carrier_id:u.id,amount:Number(d.amount),note:d.note||null,status:"pending"});if(r.error){alert(r.error.message);return}openModal('<div class="success"><div class="success-icon">✓</div><span class="eyebrow">ENVIADA</span><h2>Cotización enviada.</h2><p>El cliente verá tu oferta junto con el estado de tu vehículo. Su teléfono sigue protegido.</p><button class="primary submit" id="backLoads">Volver a cargas</button></div>');$("#backLoads").onclick=carrierLoads}
}
...
async function dailyReminder(){
 ...
 const p=await myProfile(); if(!p||p.role==="admin")return;
 const key="movelo-daily-"+u.id+"-"+new Date().toISOString().slice(0,10);
 if(localStorage.getItem(key))return;
 let title="Buenos días 👋",body="";
 const today=new Date();
 const tomorrow=new Date(today);tomorrow.setDate(today.getDate()+1);
 const iso=d=>d.toISOString().slice(0,10);
 if(p.role==="carrier"){
   const r=await sb.from("quotes").select("load_id,load_requests(cargo_type,origin_area,destination_area,requested_date,status)").eq("carrier_id",u.id).eq("status","accepted");
   const ops=(r.data||[]).map(x=>x.load_requests).filter(Boolean);
   const todayOp=ops.find(x=>x.status==="accepted"&&x.requested_date===iso(today));
   const tomorrowOp=ops.find(x=>x.status==="accepted"&&x.requested_date===iso(tomorrow));
   if(todayOp){title="🔔 Recuerda tu servicio de hoy";body="Hoy tienes un servicio: "+(todayOp.cargo_type||"transporte")+" · "+(todayOp.origin_area||"origen")+" → "+(todayOp.destination_area||"destino")+". Revisa los detalles en MOVELO."}
   else if(tomorrowOp){title="📅 Servicio de mañana";body="Mañana tienes un servicio: "+(tomorrowOp.cargo_type||"transporte")+" · "+(tomorrowOp.origin_area||"origen")+" → "+(tomorrowOp.destination_area||"destino")+". Revisa los detalles en MOVELO."}
   else {title="🚚 MOVELO para ti";body="Revisa las oportunidades nuevas de transporte, carga y acarreo que puedan encajar con tu vehículo o maquinaria."}
 }else{
   const r=await sb.from("load_requests").select("id,cargo_type,origin_area,destination_area,requested_date,status").eq("customer_id",u.id).in("status",["accepted","delivered"]);
   const ops=r.data||[];
   const todayOp=ops.find(x=>x.status==="accepted"&&x.requested_date===iso(today));
   const tomorrowOp=ops.find(x=>x.status==="accepted"&&x.requested_date===iso(tomorrow));
   if(todayOp){title="🔔 Recuerda tu servicio de hoy";body="Hoy tienes un servicio: "+(todayOp.cargo_type||"transporte")+" · "+(todayOp.origin_area||"origen")+" → "+(todayOp.destination_area||"destino")+". Revisa los detalles en MOVELO."}
   else if(tomorrowOp){title="📅 Recuerda tu servicio de mañana";body="Mañana tienes un servicio: "+(tomorrowOp.cargo_type||"transporte")+" · "+(tomorrowOp.origin_area||"origen")+" → "+(tomorrowOp.destination_area||"destino")+". Revisa los detalles en MOVELO."}
   else {title="📦 ¿Necesitas mover algo?";body="Cuando necesites transporte, carga, mudanza o acarreo, puedes publicar tu solicitud en MOVELO."}
 }
 localStorage.setItem(key,"1");
 openModal('<div class="daily-notice"><div class="success-icon">🔔</div><span class="eyebrow">RECORDATORIO MOVELO</span><h2>'+esc(title)+'</h2><p class="modal-sub">'+esc(body)+'</p><button class="primary submit" id="dailyClose">Entendido</button></div>');
 $("#dailyClose").onclick=closeModal;
}
async function render(){const u=await user(),box=$("#sessionBox");if(!u){box.innerHTML="";$("#loginNav").textContent="Entrar";await renderHeroAvailability();return}const p=await myProfile();box.innerHTML='<span>Sesión: <b>'+esc(u.email)+'</b> · '+esc(p?.role||"usuario")+'</span> <button id="msgBtn" class="secondary small">💬 Mensajes</button> <button id="notifBtn" class="secondary small">🔔</button> <button id="logoutBtn" class="secondary small">Salir</button>';$("#msgBtn").onclick=messagingCenter;$("#notifBtn").onclick=notifications;$("#logoutBtn").onclick=async()=>{await sb.auth.signOut();render();refresh()};$("#loginNav").textContent=p?.role==="admin"?"Centro de Control":"Mi cuenta";await renderHeroAvailability();setTimeout(dailyReminder,500)}

...
$("#clientBtn").onclick=$("#clientBtn2").onclick=()=>start("customer");$("#carrierBtn").onclick=$("#carrierBtn2").onclick=()=>start("carrier");$("#loginNav").onclick=async()=>{const p=await myProfile();if(p?.role==="admin")adminDashboard();else if(p?.role==="customer")customerDashboard();else if(p?.role==="carrier")carrierOperations();else authForm("customer")};$("#refreshBtn").onclick=refresh;$("#menuBtn").onclick=()=>$(".nav").classList.toggle("show");let liveChannel=null;async function connectLive(){const u=await user();if(!u){if(liveChannel){sb.removeChannel(liveChannel);liveChannel=null}return}if(!liveChannel)liveChannel=sb.channel("movelo-market").on("postgres_changes",{event:"*",schema:"public",table:"load_requests"},refresh).subscribe()}connectLive();setInterval(connectLive,5000);setInterval(refresh,15000);sb.auth.onAuthStateChange(()=>setTimeout(render,0));refresh();render();
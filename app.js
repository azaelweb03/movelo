...
function loadForm(){
 openModal('<span class="eyebrow">CLIENTE · NECESITO MOVER</span><h2>¿Qué necesitas mover?</h2><p class="modal-sub">Cuéntanos qué buscas transportar y MOVELO te muestra solo las preguntas necesarias.</p><form id="loadForm"><label>¿Qué necesitas mover?<select name="move_type" id="moveType" required><option value="">Elige una opción</option><option value="persona">👤 Persona</option><option value="animal">🐄 Animal</option><option value="carga">📦 Carga</option><option value="mudanza">🏠 Mudanza</option></select></label><div id="moveDetails"></div><label>¿Qué tipo de transporte estás buscando?<select name="transport_type" required><option value="">Elige una opción</option><option>Bicicleta</option><option>Moto</option><option>Carro</option><option>Pickup</option><option>Panel / furgón</option><option>Camión</option><option>Bus / transporte de personas</option><option>Tractor</option><option>Maquinaria</option><option>Otro</option></select></label><div class="two"><label>De<input name="origin" required placeholder="Lugar de salida"></label><label>A<input name="destination" required placeholder="Lugar de destino"></label></div><div id="quantityBox"></div><div class="two"><label>Fecha<input name="pickup_date" type="date" required></label><label>Urgencia<select name="urgency"><option>Normal</option><option>Pronto</option><option>Urgente</option></select></label></div><label>Presupuesto (opcional)<input name="budget" type="number" step="0.01" placeholder="Ej. 180"></label><label>Detalles adicionales<textarea name="notes" rows="3" placeholder="Horario, referencia, fragilidad, requisitos…"></textarea></label><button class="primary submit">Buscar transporte →</button><small class="form-note">🔒 Tu teléfono no se publica. MOVELO mantiene la comunicación protegida.</small></form>');
 const details=$('#moveDetails'), type=$('#moveType'), quantityBox=$('#quantityBox');
 const renderDetails=()=>{
   const t=type.value;
   let h='',q='';
   ...
   if(t==='animal') { h='<div class="dynamic-box"><label>¿Qué animal?<input name="cargo_detail" required placeholder="Ej. cerdos, ganado, perro"></label><label>¿Cuántos?<input name="animal_count" type="number" min="1" value="1" required></label></div>'; q='<label>Peso aproximado total (opcional)<input name="weight" placeholder="Ej. 500 kg"></label>'; }
   if(t==='carga') { h='<div class="dynamic-box"><label>¿Qué tipo de carga?<select name="cargo_detail" required><option value="">Elige</option><option>Carga seca</option><option>Agrícola</option><option>Refrigerada</option><option>Maquinaria</option><option>Líquidos</option><option>Otro</option></select></label><label>Describe la carga<input name="special_requirements" placeholder="Ej. sacos, frágil, producto agrícola…"></label></div>'; q='<label>Peso aproximado (opcional)<input name="weight" placeholder="Ej. 1,500 kg"></label>'; }
   if(t==='mudanza') { h='<div class="dynamic-box"><label>¿Qué vas a mudar?<input name="cargo_detail" required placeholder="Ej. Casa de 2 habitaciones"></label><label>¿Necesitas ayuda para cargar y descargar?<select name="loading_help" required><option value="no">No, nosotros hacemos todo</option><option value="si">Sí, necesito ayuda</option></select></label></div>'; q='<label>Peso aproximado (opcional)<input name="weight" placeholder="Ej. 1,500 kg"></label>'; }
   details.innerHTML=h; quantityBox.innerHTML=q;
 };
 type.onchange=renderDetails;
 $('#loadForm').onsubmit=async e=>{
   e.preventDefault();
   const u=await user(),d=Object.fromEntries(new FormData(e.target));
   if(validatePublicText({origin:d.origin,destination:d.destination,weight:d.weight,notes:d.notes,cargo_detail:d.cargo_detail,special_requirements:d.special_requirements}))return rejectConfidentialData();
   const move=d.move_type;
   const cargo=move==='persona'?'Personas':move==='animal'?'Animal':move==='mudanza'?'Mudanza / acarreo':(d.cargo_detail||'Carga');
   const quantity=move==='persona'?(d.people_count+' persona(s)'):move==='animal'?(d.animal_count+' animal(es)'):move==='carga'?'Carga':d.cargo_detail||'Mudanza';
   const notes=[d.notes,d.weight?'Peso aproximado: '+d.weight:null,d.transport_type?'Transporte solicitado: '+d.transport_type:null].filter(Boolean).join(' · ');
   const r=await sb.from("load_requests").insert({customer_id:u.id,move_type:move,cargo_type:cargo,origin_area:d.origin,destination_area:d.destination,requested_date:d.pickup_date,quantity,urgency:(d.urgency||"normal").toLowerCase(),budget:d.budget?Number(d.budget):null,notes:notes||null,loading_help:d.loading_help||null,cargo_detail:d.cargo_detail||null,special_requirements:d.special_requirements||null});
   if(r.error){alert(r.error.message);return}
   openModal('<div class="success"><div class="success-icon">✓</div><span class="eyebrow">PUBLICADA</span><h2>Tu necesidad ya está en MOVELO.</h2><p>Ahora los transportistas pueden verla y cotizar.</p><button class="primary submit" id="ok">Ver mis cargas</button></div>');
